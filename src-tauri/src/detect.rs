...
mod tests {
    ...
    /// Zechariah heard as "Zacchaeria" put Luke 19 on the wall, because that word is
    /// nearer Zacchaeus than Zechariah and the story index answered.
    ///
    /// The obvious fix was to argue from grammar: a word followed by "chapter 4 verse
    /// 6" can only be a book, so relax the similarity bar when that follows. It was
    /// built, measured and reverted. "Zacchaeria" scores 0.683 against "Zechariah",
    /// out of reach of any bar worth shipping, while "Mach" scores 0.886 against
    /// *Malachi* and 0.667 against Mark - so a relaxed bar turned "Mach 4:1" from
    /// matching nothing into confidently opening Malachi.
    ///
    /// Grammar proves the word is a book. It says nothing about *which* book, and
    /// that is the half that matters. So these are named in `corrections` instead.
    #[test]
    fn a_badly_misheard_book_is_named_rather_than_guessed_at() {
        let find = |said: &str| {
        ...
        // The person, with no chapter after him, is not a book.
        assert_eq!(find("zacchaeus climbed up a tree to see jesus"), None);
        // And the one the relaxed bar broke: better to resolve nothing than Malachi.
        assert_ne!(
